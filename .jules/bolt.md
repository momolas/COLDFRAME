...
## 2025-12-17 - [Swift 5.9 @Observable Property Mutations]
**Learning:** In Swift 5.9, `@Observable` properties do not automatically check for equality before calling `withMutation`. This means assigning the same value to a property in a high-frequency execution path (like `CLLocationManager` callbacks for heading or location) will unnecessarily invalidate and re-render views on the main thread, causing significant CPU overhead.
**Action:** Always guard mutations of `@Observable` properties with an explicit equality check (e.g., `if property != newValue`) when updating them inside high-frequency execution paths to prevent redundant main-thread view invalidations.
