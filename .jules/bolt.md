...
**Learning:** Swift 5.9 `@Observable` properties do not automatically check for equality before calling `withMutation`. In high-frequency paths like sensor or location callbacks, this leads to redundant main-thread view invalidations.
**Action:** Always guard mutations of `@Observable` properties with explicit equality checks (e.g., `if property != newValue`) to reduce CPU overhead.
