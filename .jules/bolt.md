...
## 2026-04-24 - [Missing Equality Checks in @Observable Mutators]
**Learning:** Swift 5.9 @Observable properties do not automatically check for equality before calling withMutation. Mutating them in high-frequency callbacks (like CoreLocation delegates) causes massive CPU overhead and redundant view invalidations.
**Action:** Always guard mutations of @Observable properties with an explicit equality check (e.g., if property != newValue) inside high-frequency execution paths.
