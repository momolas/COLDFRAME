...
## 2025-10-24 - Avoiding Redundant @Observable Invalidations
**Learning:** Swift 5.9 `@Observable` properties do not automatically check for equality before calling `withMutation`. Mutating these properties in high-frequency execution paths (like `CLLocationManager` sensor callbacks) without explicit equality checks causes redundant main-thread view invalidations and massive CPU overhead.
**Action:** Always guard mutations of `@Observable` properties with an explicit equality check (e.g., `if property != newValue`) when updating them inside high-frequency execution paths.
